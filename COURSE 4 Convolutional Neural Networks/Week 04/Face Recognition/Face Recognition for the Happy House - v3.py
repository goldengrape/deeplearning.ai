...
def triplet_loss(y_true, y_pred, alpha = 0.2):
    ...
    neg_dist = tf.reduce_sum(tf.square(tf.subtract(negative,anchor)))
    # Step 3: subtract the two previous distances and add alpha.
    basic_loss = tf.reduce_sum(tf.subtract(pos_dist,neg_dist)+alpha)
...
def verify(image_path, identity, database, model):
    ...
    dist = np.linalg.norm((encoding-database[identity]),axis=-1,keepdims=False)[0]
...
def who_is_it(image_path, database, model):
    ...
    for (name, db_enc) in database.items():
        ...
        dist = np.linalg.norm((encoding-db_enc),axis=-1,keepdims=False)[0]
